...
PUBLIC int do_rdwt() {
    int fd = fs_mag.FD;         // file descriptor
    void * buf = fs_mag.BUF；   // r/w buffer
    int len = fs_mag.CNT;       // r/w bytes
    ...
    if (imode == I_CHAR_SPECIAL) {
        int t = fs_mag.type == READ ? DEV_READ : DEV_WRITE;
        fs_mag.type = t;
        
        int dev = pin->i_start_sect;
        assert(MAJOR(dev) == 4);
        
        fs_mag.DEVICE = MINOR(dev); 
        fs_mag.BUF = buf;
        fs_mag.CNT = len;
        fs_mag.PROC_NR = src;   
        assert(dd_map[MAJOR(dev)].driver_nr != INVALID_DRIVER);
        send_recv(BOTH, dd_map[MAJOR(dev)].driver_nr, &fs_mag);
        assert(fs_mag.CNT == len);
        
        return fs_mag.CNT;
    } else {
        assert(pin->i_mode == I_REGULAR || pin->->i_mode == I_DIRECTORY);
        assert((fs_mag.type == READ) || (fs_mag.type == WRITE));
        int pos_end;
        if (fs_mag.type == READ) {
        ...
        int re_sect_min = pin->i_start_sect + (pos >> SECTOR_SIZE_SHIFT);
        int re_sect_max = pin->i_start_sect + (pos_end >> SECTOR_SIZE_SHIFT);
