 ...
 * Remove a file.
 *
 * @note We clear the i-node in inode_array[] although it is really needed.
...
PUBLIC int do_unlink() {
    ...
    if (pin->i_mode != I_REGULAR) {     // can only remove regular file.
    ...
    assert(byte_idx < SECTOR_SIZE);     // we have only ont i-map sector.
    ...
    // current sector nr.
    int s = 2 /* 2: bootsect + superblk */ + sb->nr_imap_sects + byte_idx / SECTOR_SIZE;
    RD_SECT(pin->i_dev, s);
    ...
    for (int k; k < bits_cnt; k++, i++, bits_left-=8) {
        if (i ==SECTOR_SIZE) {
            i = 0;
            WR_SECT(pin->i_dev, s);
            RD_SECT(pin->i_dev, ++s);
